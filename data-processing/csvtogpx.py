...
for csvFile in csvFiles:
    ...
    for row in csvData:
        if rowNum == 0:
            ...
            xmlData.write('\t'+'<wpt lat="'+latConvert(row[1])+'" lon="'+lonConvert(row[2])+'">'+'\n')
            xmlData.write('\t\t'+'<ele>'+row[3]+'</ele>'+'\n')
            xmlData.write('\t\t'+'<time>'+row[0]+'</time>'+'\n')
            xmlData.write('\t\t'+'<name>'+row[9]+'</name>'+'\n')
            xmlData.write('\t\t'+'<sym>Dot</sym>'+'\n')
            xmlData.write('\t\t'+'<type>waypoint</type>'+'\n')
            xmlData.write('\t\t'+'<sat>'+row[8]+'</sat>'+'\n')
            xmlData.write('\t\t'+'<fix>'+fix(row[7], row[8])+'</fix>'+'\n')
